...
export default function AgendaPage() {
  ...
  const cancelMutation = useMutation({
    mutationFn: (id: string) => appointmentsApi.cancel(token!, id, "Cancelado pela profissional"),
    onSuccess: () => {
  ...
  return (
    <div>
      <h1 className="text-xl font-semibold text-brand-900">Sua agenda</h1>
      <p className="mt-1 text-sm text-neutral-500">Horários agendados pelas suas clientes.</p>

      <div className="mt-6 space-y-6">
        {isLoading && <p className="text-sm text-neutral-400">Carregando...</p>}

        {!isLoading && grouped.length === 0 && (
          <Card className="text-center text-sm text-neutral-400">Nenhum agendamento por enquanto.</Card>
        )}
                ...
                <Card key={appt.id} className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className="text-sm text-neutral-500">
                      Cliente: {appt.client?.name} {appt.client?.phone ? `· ${appt.client.phone}` : ""}
                    </p>
